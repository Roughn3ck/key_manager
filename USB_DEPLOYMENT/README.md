...
- **Write Operations with Confirmation**: Advanced mode shows "Compound Fees", "Collect Fees", and "Withdraw Fees" buttons on LP cards. Every write operation requires explicit user confirmation dialog. Writer is a tool, not an autonomous agent.
- **Collect Fees / Withdraw Fees**: Both buttons call the same Uniswap V3 `collect()` function on the Project X PositionManager to transfer uncollected fees to your wallet; "Withdraw" is provided as an alternative label for users who find it clearer than "Collect". Collecting fees from a Project X HyperEVM LP position has been tested and confirmed working end-to-end via the embedded agent.
