...
- A stale EXE is worse than no EXE — never leave `USB_DEPLOYMENT/coldstack.exe` stale

**When NOT to build:**
- During intermediate prompts in a multi-prompt workflow (each prompt will say "do NOT rebuild")
- If the build fails — fix the error before considering the task done
...
- `lp_engine.py` — LP position aggregation facade (`LPPosition` dataclass, `VenueAdapter` ABC, `StrategyEngine`, `LPEngine`). Read-only.
  - `venue_adapters/` — One adapter per venue, auto-registered on import (`__init__.py`). `hyperliquid_adapter.py` (read: HyperEVM NFT Position Manager + L1 perp/spot), `hyperliquid_writer.py` (write: wrap/unwrap/approve/open/increase/decrease/collect/close/rebalance, signs via agent on :8842), `krystal_adapter.py` (read skeleton: BSC / PancakeSwap V3 via direct RPC reads, multichain), `venue_writer.py` (writer ABC).
...
- No password storage — password is held in memory only during active session.
- On lock/session expiry: clear `current_password`, `revealed_mnemonics`, and `address_db` references.
- LP Engine is read-only — never add signing or transaction execution capabilities to the adapter.
- Go Online is OFF by default — never change this default.
- The writer signs via the key_manager_agent (localhost:8842) — never load private keys into the GUI or writer directly.
...
1. HyperEVM swap router address not yet confirmed — `swap()` raises `NotImplementedError`; cross-ratio rebalances and compound-with-swap deferred
...
3. G5 `lp_address` in `strategy.json` is 66 chars (not a valid 42-char EVM address) — needs clarification for HyperEVM adapter testing
4. Compound/Collect fees may fail with "Agent error (broadcast_transaction) 'result'" if the embedded key_manager_agent is not running or not unlocked — ensure vault is unlocked and the agent thread started

## Testing

- `test_derivation.py` — BIP39 derivation parity + multi-chain tests
- `python3 -m py_compile src/<file>.py` — syntax check after editing any source file
- LP Engine: `python3 -c "from lp_engine import LPEngine; e = LPEngine(online_mode=False); e.fetch_all_positions('0x...')"` → should raise `OfflineError`
- Always run `python3 -m py_compile src/<file>.py` after editing any source file

## Project State Tracking

`STATUS.md` tracks per-version changelogs, known issues, and next steps. `README.md` has the full project documentation. Both are updated with each release — keep them in sync with code changes.